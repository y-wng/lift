...
def maybe_download(url: str, *, force_download: bool = False, **kwargs) -> pathlib.Path:
    ...
    try:
        ...
        with filelock.FileLock(lock_path):
            ...
            _download_fsspec(url, scratch_path, **_configure_fsspec_kwargs(url, kwargs))
...
def _configure_fsspec_kwargs(url: str, kwargs: dict) -> dict:
    """Adjust fsspec kwargs for the current environment.

    For GCS downloads, gcsfs creates an aiohttp ClientSession. aiohttp does not read
    HTTP(S) proxy variables unless `trust_env=True`, so opt in automatically when the
    user has already configured proxy environment variables in the shell.
    """
    parsed = urllib.parse.urlparse(url)
    if parsed.scheme not in {"gs", "gcs"}:
        return kwargs
    if not any(os.getenv(var) for var in _PROXY_ENV_VARS):
        return kwargs

    direct_session_kwargs = kwargs.get("session_kwargs")
    if isinstance(direct_session_kwargs, dict) and (
        "trust_env" in direct_session_kwargs or "proxy" in direct_session_kwargs
    ):
        return kwargs

    nested_protocol_kwargs = kwargs.get(parsed.scheme)
    if isinstance(nested_protocol_kwargs, dict):
        nested_session_kwargs = nested_protocol_kwargs.get("session_kwargs")
        if isinstance(nested_session_kwargs, dict) and (
            "trust_env" in nested_session_kwargs or "proxy" in nested_session_kwargs
        ):
            return kwargs

    updated_kwargs = dict(kwargs)
    updated_session_kwargs = dict(direct_session_kwargs or {})
    updated_session_kwargs["trust_env"] = True
    updated_kwargs["session_kwargs"] = updated_session_kwargs
    logger.info("Detected proxy environment variables; enabling trust_env for GCS download session")
